...
import allStops from "../../public/data/stop_times.json" with { type: "json" };
...
const generateTripData = async () => {
    const tripIdToHash = {}
    allStops.forEach(({ trip_id, arrival_time, stop_id }) => {
        tripIdToHash[trip_id] ??= {}
        tripIdToHash[trip_id][stop_id] = arrival_time
    })
...
const writeToDisk = async (filename, data) => {
    ...
    const path = `./public/process-data/${filename}`
